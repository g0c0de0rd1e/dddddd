...
- [x] Реализовать страницу управления нейросотрудником с табами (инструкция, база знаний, расписание, настройки, интеграции, каналы, тестирование)
- [x] Подключить мастер создания нейросотрудника к API (создание черновика, сохранение шага выбора каналов)
- [x] Добавить drag-n-drop менеджер базы знаний с предпросмотром, ограничениями форматов и синхронизацией с API
- [x] Реализовать загрузку и выбор аватаров для ассистентов и пользователя (интеграция со стореджем)
- [x] Создать детальную страницу готового ассистента (просмотр доступных параметров, быстрый запуск тестового чата, кнопка "Подключить")
- [x] Реализовать страницу профиля пользователя с редактированием ФИО, контактов и сменой пароля
- [x] Настроить таб "Инструкция": формы промта, креативности, задержки ответа, генерация промта по описанию
- [x] Настроить таб "База знаний": список файлов с поиском, кастомные инструкции, удаление, превью
...
- [x] Реализовать ограниченное редактирование полей для готовых ассистентов (в README выделены управляющие поля)
...
- [x] Встроить управление интеграциями и каналами через /api/v1/integrations и связанные ручки
- [x] Добавить вызовы API для промтов (обновление настроек ассистента, генерация промта)
- [x] реализовать API отправки тестовых сообщений / симуляции диалога с ассистентом
- [x] Подключить API обновления статусов ассистента (публикация, активация, стоп-слова)
- [x] Синхронизировать мастер создания с эндпоинтом /api/v1/assistants (POST, PUT) и шагами интеграций
...
- [x] Настроить состояние и кэш для табов ассистента (React Query + Zustand, оптимистичные апдейты)
- [x] Добавить хранение черновиков форм (autosave) при редактировании ассистента
